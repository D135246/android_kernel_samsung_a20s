...
static int tp_get_boot_mode(void)
{
...
static int fts_reset(int hdelayms)
{
    FTS_DEBUG("tp reset");
...
int fts_reset_proc(int hdelayms)
{
	int ret;
	FTS_FUNC_ENTER();
	mutex_lock(&fts_data->reset_lock);
	/* request reset gpio */
...
err_reset_gpio_req:
	mutex_unlock(&fts_data->reset_lock);
	FTS_FUNC_EXIT();
	return ret;
}
...
int fts_read_bootid(struct fts_ts_data *ts_data, u8 *id)
{
    int ret = 0;
    u8 chip_id[2] = { 0 };
    u8 id_cmd[4] = { 0 };
...
#if FTS_POWER_SOURCE_CUST_EN
int  fts_power_source_ctrl(struct fts_ts_data *power_idev, bool status)
{  
    int rc = 0;
    
    FTS_FUNC_ENTER();
    if (IS_ERR_OR_NULL(power_idev->vddp) || IS_ERR_OR_NULL(power_idev->vddn)) {
    ...
    if (status) {
        if (power_idev->power_disabled) {
            FTS_DEBUG("regulator enable !");
            rc = regulator_enable(power_idev->vddp);
            if ( rc != 0 ) {
                FTS_ERROR("enable vsp fail\n");
            }
            rc = regulator_enable(power_idev->vddn);
            if ( rc != 0 ) {
                FTS_ERROR("enable vsn fail\n");
            }
            power_idev->power_disabled = false;
        }
    } else {
        if (!power_idev->power_disabled) {
            FTS_DEBUG("regulator disable !");
            rc = regulator_disable(power_idev->vddp);
            if ( rc != 0 ) {
                FTS_ERROR("enable vsp fail\n");
            }
            rc = regulator_disable(power_idev->vddn);
            if ( rc != 0 ) {
                FTS_ERROR("enable vsn fail\n");
            }
            power_idev->power_disabled = true;
        }
    }
...
static int fts_power_source_exit(struct fts_ts_data *ts_data)
{
    ...
    fts_power_source_init(ts_data, DISABLE);
...
static int fts_power_source_init(struct fts_ts_data *power_idev, bool on)
{
    int rc = 0;
    FTS_FUNC_ENTER();
    if ( !on ) {
		FTS_INFO("power_init is deny\n");
        goto pwr_deny;
	}
    power_idev->vddp = regulator_get(&power_idev->spi->dev, "vddp");
	if ( IS_ERR(power_idev->vddp) ) {
        rc = PTR_ERR(power_idev->vddp);
		FTS_ERROR("Regulator get failed power_idev->vddp rc=%d\n",rc);
		goto lab_pwr_err;
    }
    power_idev->vddn = regulator_get(&power_idev->spi->dev, "vddn");
	if ( IS_ERR(power_idev->vddn) ) {
        rc = PTR_ERR(power_idev->vddn);
		FTS_ERROR("Regulator get failed power_idev->vddn rc=%d\n",rc);
		goto ibb_pwr_err;
    }
    FTS_FUNC_EXIT();
    return rc;
...
ibb_pwr_err:
	if (power_idev->vddn) {
		regulator_put(power_idev->vddn);
		power_idev->vddn = NULL;
	}
lab_pwr_err:
	if (power_idev->vddp) {
		regulator_put(power_idev->vddp);
		power_idev->vddp = NULL;
	}
    FTS_FUNC_EXIT();
    return rc;
...
*/
    ...
    ret = fts_power_source_init(ts_data, ENABLE);
    if (ret) {
        FTS_ERROR("fail to get power(regulator)");
        goto err_power_init;
    }
...
{
    //struct fts_ts_data *ts_data = dev_get_drvdata(&ts_data->dev);
    ...
    devm_tid_unregister(ts_data->dev, tid);
    ...
    devm_tid_and_ops_free(ts_data->dev, tid);
    kfree_safe(ts_data->pdata);
    kfree_safe(ts_data);
...
static int fts_ts_resume(struct device *dev)
{
    ...
    if (!ts_data->ic_info.is_incell) {
        ...
        if (ret < 0) {
            FTS_ERROR("power enter resume fail\n");
        }
#endif
        fts_reset_proc(200);  
    }
...
int tid_reset(struct device *dev)
{
    return fts_reset_proc(200);
}
...
static void get_fw_ver_bin(void *device_data)
{
	...
	char buff[16] = { 0 };
	struct touch_info_dev *tid = fts_data->tid;
	//tid->phone_fw_version = 0x05;
	...
	input_info(true, &ts_data->spi->dev, "%s: %s\n", __func__, buff);
}
...
static void aot_enable(void *device_data)
{
    ...
    ret = gesture_set_capability(&ts_data->tid->dev, mask);
    if (ret < 0) {
        input_err(true, &ts_data->spi->dev, "%s: failed. ret: %d\n",
                    __func__, ret);
...
static int fts_ts_probe(struct spi_device *spi)
{
	...
	char* cmdline_fts_tp;
	char* module_name;
    ret = tp_get_boot_mode();
		if(ret == 1){
		FTS_INFO("offcharge mode,do not load tp probe ");
		return -1;
		}
    FTS_INFO("Touch Screen(SPI BUS) driver prboe...");
	//return 0;
	
	cmdline_fts_tp = strstr(saved_command_line,"ft8006p_");
	FTS_INFO("cmdline_fts_tp= %s",cmdline_fts_tp);
	if (cmdline_fts_tp == NULL){
		FTS_ERROR("get fts,ft8006p_fail,this is not focal tp");
		return -ENOMEM;
		}
	module_name = cmdline_fts_tp + strlen("ft8006p_");
	FTS_INFO("module_name= %s",module_name);

	if(strncmp(module_name,"tianma_hdp_video",strlen("tianma_hdp_video"))!= 0){
		FTS_INFO("is not focal TP \n");
		return -ENOMEM;
		}
    spi->mode = SPI_MODE_1;
    spi->bits_per_word = 8;
    ...
    if (unlikely(!tid)) {
	ret = -ENOMEM;
        goto ts_data_alloc_failed;
    }
    tid_ops = tid->tid_ops;
    ts_data->tid = tid;
    dev_set_drvdata(&spi->dev, ts_data);
    ...
    if (unlikely(ret)){
	FTS_ERROR("tid register failed");
        goto tid_and_ops_alloc_failed;
    }
/*	
    ret = tid_upgrade_firmware_nowait(tid);
    if (unlikely(ret))
	FTS_ERROR("tid upgrade firmware failed");
*/
#ifdef SEC_TSP_FACTORY_TEST
		ret = sec_cmd_init(&ts_data->sec, fts_commands,
...
tid_register_failed:
	devm_tid_unregister(&spi->dev, tid);
tid_and_ops_alloc_failed:
	devm_tid_and_ops_free(&spi->dev, tid);
ts_data_alloc_failed:
	kfree_safe(ts_data);

	FTS_INFO("Touch Screen(I2C BUS) driver probe failed");
	return ret;
}
